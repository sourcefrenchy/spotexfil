"""crypto.py - Payload encoding, encryption, and integrity verification.
...
KEY_SIZE = _PROTO['crypto']['key_size']
NONCE_SIZE = _PROTO['crypto']['nonce_size']
SALT_SIZE = _PROTO['crypto']['salt_size']
KDF_ITERATIONS = _PROTO['crypto']['kdf_iterations']

# Compression flag bytes prepended to payload
FLAG_COMPRESSED = bytes([_PROTO['compression']['flag_compressed']])
FLAG_RAW = bytes([_PROTO['compression']['flag_raw']])
...
class Subcipher(CryptoProvider):
    ...
    Implements the CryptoProvider interface.
    """

    def __init__(self, spot=None, encryption_key: str = None,
                 compress: bool = True):
        """Initialize Subcipher.

        Args:
            spot: Optional Spot instance (unused, kept for compat).
