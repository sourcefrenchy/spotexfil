"""Tests for transport.py - Spotify API wrapper with mocked API calls."""
...
class TestAuthentication:
    def test_missing_env_var_exits(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch('spotexfil.transport.load_config', return_value={}):
...
class TestConfigFile:
    def test_config_file_loads(self):
        ...
        from spotexfil.transport import load_config
        ...
        try:
            with patch('spotexfil.transport.CONFIG_PATHS', [path]):
                result = load_config()
    ...
    def test_config_file_missing_returns_empty(self):
        ...
        from spotexfil.transport import load_config
        with patch('spotexfil.transport.CONFIG_PATHS',
                   ['/nonexistent/.conf']):
            result = load_config()
        assert result == {}
...
class TestClearData:
    ...
    def test_clears_cover_name_playlists(self, spot_cover, mock_spotipy):
        ...
        from spotexfil.transport import MARKER_SEP
        mock_spotipy.user_playlists.return_value = {
            'items': [
